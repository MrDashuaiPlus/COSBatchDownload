...
public class DownloadManager {
	...
	private DownloadManager() {
		mThreadPool = new DownloadThreadPool(DEFAULT_CORE_POOL_SIZE, DEFAULT_MAX_POOL_SIZE, DEFAULT_KEEP_ALIVE_TIME,
	...
	public void addMission(DownloadMission downloadTask) {
		if (downloadTask.mSaveName == null)
			return;
	...
	public Boolean isAllFileFinished() {
		for (Integer mission_id : mMissions.keySet()) {
			if (isFileFinished(mission_id) == false) {
				return false;
			}
		}
		return true;
	...
	public Boolean isFileFinished(int mission_id) {
		DownloadMission mission = mMissions.get(mission_id);
		return mission.isFileFinished();
	}
	...
	public double getDownloadPercent() {
		...
		return 100.0*getTotalDownloadedSize()/getTotalFileSize();
	}
	
	public String getReadableDownloadSize() {
		return DownloadUtils.getReadableSize(getTotalDownloadedSize());
	}
	public String getReadableTotalFileSize() {
		return DownloadUtils.getReadableSize(getTotalFileSize());
	}
